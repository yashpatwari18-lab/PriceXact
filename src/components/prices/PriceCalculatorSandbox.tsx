  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="bg-white dark:bg-[#141A17] rounded-2xl p-6 sm:p-8 border border-stone-200/80 dark:border-stone-800 shadow-sm space-y-4">
        <div className="max-w-3xl space-y-1.5">
          ...
          <h1 className="font-serif text-2xl sm:text-3xl font-bold text-stone-900 dark:text-white">
            10% Trimming & Statistical Engine Sandbox
          </h1>
          <p className="text-xs sm:text-sm text-stone-500 dark:text-stone-400 leading-relaxed">
            Execute the mathematical data-cleaning pipeline: sorting crowdsourced prices, trimming lowest & highest 10% outliers, and computing the trimmed arithmetic mean and dispersion standard deviation (σ).
          </p>

          {/* Quick Preset Buttons */}
          <div className="pt-2 flex flex-wrap gap-2 text-xs items-center">
            <span className="text-stone-400 font-medium text-[11px]">Benchmark Datasets:</span>
            <button
              onClick={() => loadPreset('farmer_paper')}
              className="px-2.5 py-1 bg-stone-100 dark:bg-stone-800/80 hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-800 dark:text-stone-200 rounded-md font-medium text-[11px] transition-colors"
            >
              Farmer Sample (15 records · Target: ₹9.18)
            </button>
            <button
              onClick={() => loadPreset('consumer_paper')}
              className="px-2.5 py-1 bg-stone-100 dark:bg-stone-800/80 hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-800 dark:text-stone-200 rounded-md font-medium text-[11px] transition-colors"
            >
              Consumer Sample (10 records · Target: ₹47.25)
            </button>
            <button
              onClick={() => loadPreset('volatile_mandi')}
              className="px-2.5 py-1 bg-stone-100 dark:bg-stone-800/80 hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-800 dark:text-stone-200 rounded-md font-medium text-[11px] transition-colors"
            >
              Mandi Dispersion Sample
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left Input Panel */}
        <div className="lg:col-span-5 bg-white dark:bg-[#141A17] rounded-2xl p-6 border border-stone-200/80 dark:border-stone-800 shadow-sm space-y-5">
          <div>
